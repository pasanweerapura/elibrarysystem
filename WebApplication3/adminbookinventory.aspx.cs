        ...
        /* void addNewBook()
         {
             try
             {
                 ...
                 foreach (int i in ListBox1.GetSelectedIndices())
                 {
                     genres = genres + ListBox1.Items[i].Text + ","; // Get the text of the selected items
                 }
                 genres = genres.Remove(genres.Length - 1);
                 ...
                 if (FileUpload1.HasFile)
                 {
                     ...
                     // Generate a unique filename (you can use a GUID or timestamp)
                     string uniqueFileName = Guid.NewGuid().ToString() + extension;

                     // Set the file path
                     string filepath = "~/book_inventory/" + uniqueFileName;
